...
def expand_save_folder_dirs(rootdir, args):
    latent_seed_pretrain = str(args.latent_dim) + \
                           "_" + str(args.seed) + \
                           "_" + args.pretrain_algo + \
                           "_" + str(args.expert_reward_weight) + \
                           "_" + str(args.controller_coef)
...
def main():
    ...
    if args.load_model:
        if args.gail and args.icm:
            exploitation_actor_critic, exploration_actor_critic, discr, intr, ob_rms = \
                torch.load(os.path.join(pretrain_dir, args.env_name + ".pt"), map_location=device)

            reset_model_machines(intr, device)
            reset_model_machines(discr, device)
        else:
            exploitation_actor_critic, exploration_actor_critic, ob_rms = \
                torch.load(os.path.join(pretrain_dir, args.env_name + ".pt"), map_location=device)
        ...
        if args.icm:
            intr = icm.IntrinsicCuriosityModule(envs.observation_space.shape, envs.action_space, args.latent_dim, 100,
                                                lr=args.lr, r_coef=args.intr_coef,
                                                favor_zero_expert_reward=args.favor_zero_expert_reward, device=device)
    ...
    reset_model_machines(exploitation_actor_critic, device)
    reset_model_machines(exploration_actor_critic, device)
    ...
    if args.gail:
        if args.env_name == 'CarlaUrbanIntersection-v0':
            ...
            file_name = os.path.join('/home/hsyoon/job/vilab-pytorch-rl_exploration/pytorch-a2c-ppo-acktr-gail-master/gail_experts/data-20200225_SH_11-100.pt')
    ...
    for j in range(num_updates):
                ...
                if 'Carla' in args.env_name:
                    if j % 10 == 0 and j != 0:
                        plt_value, plr_value, action, action_raw, plt_action_log_prob, plr_action_log_prob, plt_recurrent_hidden_states, plr_recurrent_hidden_states, agent_type, plt_action, plr_action, plt_action_raw, plr_action_raw = \
                            controller(args.env_name, exploitation_actor_critic, exploration_actor_critic, args.controller_coef, exploitation_rollouts.obs[step], exploration_rollouts.obs[step], exploitation_rollouts.recurrent_hidden_states[step],
        ...
        if (j % args.save_interval == 0 or j == num_updates - 1) and args.save_dir != "":
            ...
            eval_logs_data = np.hstack([_eval_epochs, _eval_mean_rewards, _eval_median_rewards, _eval_min_rewards, _eval_max_rewards])
            ...
            if args.gail and args.icm:
                torch.save([
                    exploitation_actor_critic, exploration_actor_critic, discr, intr,
                    getattr(utils.get_vec_normalize(envs), 'ob_rms', None)
                ], os.path.join(save_dir, args.env_name + "_" + str(j) + ".pt"))
            else:
                torch.save([
                    exploitation_actor_critic, exploration_actor_critic,
                    getattr(utils.get_vec_normalize(envs), 'ob_rms', None)
                ], os.path.join(save_dir, args.env_name + "_" + str(j) + ".pt"))
