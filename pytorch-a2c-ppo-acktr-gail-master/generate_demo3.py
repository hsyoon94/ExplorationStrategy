        ...
        while termination is False:
            ...
            for actor_snapshot in world_snapshot:
                ...
                if 'vehicle.audi.tt' in actual_actor.type_id:
                    ...
                    tmp_obs[6] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2))  # speed, not velocity
                    tmp_obs[7] = 1.0 - float(abs(actor_snapshot.get_transform().location.x - destination_location.x) + abs(actor_snapshot.get_transform().location.y - destination_location.y)) / float(abs(world.location_mycar.x - destination_location.x) + abs(world.location_mycar.y - destination_location.y))
                    ...
                    # x > 0 and y <= -25 or x <= -25 or y >= 0 and x >= 0 then out!
                    if (actor_snapshot.get_transform().location.x > 0 and actor_snapshot.get_transform().location.y <= -25) or actor_snapshot.get_transform().location.x <= -25 or (actor_snapshot.get_transform().location.y >= 0 and actor_snapshot.get_transform().location.x >= 0):
                        save = False
                        termination = True
                        episode = episode + 1
