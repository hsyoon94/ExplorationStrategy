...
class MountainOldCarWithoutVelEnv(gym.Env):
    metadata = {
    ...
    def step(self, action):
        ...
        #     self.velocity += -self.force + math.cos(3*position)*(-self.gravity)
        # elif action == 1:  # push right
        #     self.velocity += +self.force + math.cos(3*position)*(-self.gravity)
        self.velocity += (action - 1) * self.force + math.cos(3 * position) * (-self.gravity)
        self.velocity = np.clip(self.velocity, -self.max_speed, self.max_speed)
        position += self.velocity
        position = np.clip(position, self.min_position, self.max_position)
        if (position==self.min_position and self.velocity<0.0): self.velocity = 0.0
