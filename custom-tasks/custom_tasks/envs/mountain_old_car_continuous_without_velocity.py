...
class MountainOldCarContinuousWithoutVelEnv(gym.Env):
    metadata = {
    ...
    def step(self, action):
        ...
        self.velocity += force*self.power -0.0025 * math.cos(3*position)
        if (self.velocity > self.max_speed): self.velocity = self.max_speed
        if (self.velocity < -self.max_speed): self.velocity = -self.max_speed
        position += self.velocity
        if (position > self.max_position): position = self.max_position
        if (position < self.min_position): position = self.min_position
        if (position==self.min_position and self.velocity<0): self.velocity = 0
