...
class CarlaUrbanIntersectionEnv8425_eval(gym.Env):
    metadata = {
        'render.modes': ['human']
    }
    ...
    def __init__(self):
        ...
        self.observation_space = np.array([0.0 for i in range(1 * 36)])
        self.reward = None
    ...
    def step(self, action):
        ...
        destination_distance = math.sqrt(diff_x + diff_y)
        ...
        new_collision = self.collision_curr - self.collision_prev
        new_invasion = self.invasion_curr - self.invasion_prev

        if new_collision > 0 and self.collision_prev == self.collision_prev_prev:
            reward = reward - 3

        if new_invasion > 0 and self.invasion_prev == self.invasion_prev_prev:
            if next_obs[0] != 29 and next_obs[0] != 30 and next_obs[0] != 37:
                reward = reward - 3

        self.collision_prev_prev = self.collision_prev
        self.collision_prev = self.collision_curr
        self.collision_curr = len(collisions_hist)

        self.invasion_prev_prev = self.invasion_prev
        self.invasion_prev = self.invasion_curr
        self.invasion_curr = len(invasions_hist)
