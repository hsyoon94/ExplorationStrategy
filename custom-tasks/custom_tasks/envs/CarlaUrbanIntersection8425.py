...
class CarlaUrbanIntersectionEnv8425(gym.Env):
    metadata = {
        'render.modes': ['human']
    }
    ...
    def __init__(self):
        ...
        self.observation_space = np.array([0.0 for i in range(1 * 8)])
        self.reward = None
        ...
        self.full_success_count = 0
        self.half_success_count = 0
    ...
    def reset(self):
        ...
        for actor_snapshot in world_snapshot_next:
            ...
            if 'vehicle.audi.tt' in actual_actor.type_id:
                ...
                # next_obs[0:5] = mycar_grid(actor_snapshot.get_transform().location.x, actor_snapshot.get_transform().location.y)
                # # next_obs[5] = actor_snapshot.get_transform().rotation.yaw  # orientation
                # next_obs[5] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10  # speed, not velocity
                # next_obs[6] = 1.0 - float(abs(actor_snapshot.get_transform().location.x - self.location_end.x) + abs(actor_snapshot.get_transform().location.y - self.location_end.y)) / float(
                #     abs(self.location_mycar.x - self.location_end.x) + abs(self.location_mycar.y - self.location_end.y))
                ...
                next_obs[1] = lng
                next_obs[2] = ori
                next_obs[3] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10
        ...
        for actor_snapshot in world_snapshot_next:
            ...
            #         # next_obs[7] = othercar_rad_grid(actor_snapshot.get_transform().location.x - actor_snapshot_mycar.get_transform().location().x, actor_snapshot.get_transform().location.y - actor_snapshot_mycar.get_transform().location().y)  # grid location
            #         next_obs[7] = othercar_rad_grid(actor_snapshot.get_transform().location.x, actor_snapshot.get_transform().location.y)  # grid location
            #     else:
            #         next_obs[7] = 2.0
            #     next_obs[8] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10  # speed, not velocity
            #
            # if 'vehicle.bmw.grandtourer' in actual_actor.type_id:
            #     if math.sqrt(math.pow(actor_snapshot.get_transform().location.x, 2) + math.pow(actor_snapshot.get_transform().location.y, 2)) <= 25.0:
            #         next_obs[9] = othercar_rad_grid(actor_snapshot.get_transform().location.x, actor_snapshot.get_transform().location.y)  # grid location
            #     else:
            #         next_obs[9] = 2.0
            #     next_obs[10] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10  # speed, not velocity
            if 'vehicle.tesla.model3' in actual_actor.type_id:
                if math.sqrt(math.pow(actor_snapshot.get_transform().location.x, 2) + math.pow(actor_snapshot.get_transform().location.y, 2)) <= 25.0:
                    next_obs[4] = othercar_rad_grid(actor_snapshot.get_transform().location.x - actor_snapshot_mycar.get_transform().location.x,
                                                   actor_snapshot.get_transform().location.y - actor_snapshot_mycar.get_transform().location.y)  # grid location
                else:
                    next_obs[4] = 2.0
            ...
            if 'vehicle.bmw.grandtourer' in actual_actor.type_id:
                if math.sqrt(math.pow(actor_snapshot.get_transform().location.x, 2) + math.pow(actor_snapshot.get_transform().location.y, 2)) <= 25.0:
                    next_obs[6] = othercar_rad_grid(actor_snapshot.get_transform().location.x - actor_snapshot_mycar.get_transform().location.x,
                                                   actor_snapshot.get_transform().location.y - actor_snapshot_mycar.get_transform().location.y)  # grid location
                else:
                    next_obs[6] = 2.0
                # tmp_obs[12] = actor_snapshot.get_transform().rotation.yaw  # orientation
                next_obs[7] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10  # speed, not velocity

            # if 'vehicle.chevrolet.impala' in actual_actor.type_id:
            #     if math.sqrt(math.pow(actor_snapshot.get_transform().location.x, 2) + math.pow(actor_snapshot.get_transform().location.y, 2)) <= 25.0:
            #         next_obs[11] = othercar_rad_grid(actor_snapshot.get_transform().location.x, actor_snapshot.get_transform().location.y)  # grid location
            #     else:
            #         next_obs[11] = 2.0
            #     next_obs[12] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10  # speed, not velocity
            #
            # if 'vehicle.ford.mustang' in actual_actor.type_id:
            #     if math.sqrt(math.pow(actor_snapshot.get_transform().location.x, 2) + math.pow(actor_snapshot.get_transform().location.y, 2)) <= 25.0:
            #         next_obs[13] = othercar_rad_grid(actor_snapshot.get_transform().location.x, actor_snapshot.get_transform().location.y)  # grid location
            #     else:
            #         next_obs[13] = 2.0
            #     next_obs[14] = math.sqrt(math.pow(actor_snapshot.get_velocity().x, 2) + math.pow(actor_snapshot.get_velocity().y, 2)) / 10  # speed, not velocity
    ...
    def step(self, action):
        ...
        # final_destination_distance = math.sqrt(diff_y)

        mycar_velocity = math.sqrt(math.pow(self.mycar.get_velocity().x, 2) + math.pow(self.mycar.get_velocity().y, 2))
        ...
        reward = reward + (mycar_velocity / 10)
        ...
        if math.sqrt(math.pow(actor_snapshot_mycar.get_transform().location.x, 2) + math.pow(actor_snapshot_mycar.get_transform().location.y, 2)) <= 17.5:
            reward = reward - 3
            print("INVASION TYPE 3!!")
            info['bad_transition'] = True
            self.step_count = 0
            done = True
        ...
        if final_destination_distance <= done_distance:
            ...
            self.full_success_count = self.full_success_count + 1
            print("FULL SUCCESS COUNT :", self.full_success_count)
            self.step_count = 0
            done = True

        if middle_dist <= 5 and self.reach_middle_point is False:
            self.reach_middle_point = True
            self.half_success_count = self.half_success_count + 1
            print("GET MIDDLE POINT!!")
            print("HALF SUCCESS COUNT :", self.half_success_count)
            reward = reward + 5
        ...
        # print("tmp_obs_mycar", next_obs[0:4])
        # print("tmp_obs_othercar1", next_obs[4:6])
        # print("tmp_obs_othercar2", next_obs[6:8])
